...
if ~X.bc
    M = M2;
    [C,S,k] = mex_soil(theta_res,theta_sat,M,alp,h,N,scalingfactor,ksat,l);
    X.theta = S.*(X2.theta_sat-X2.theta_res)+X2.theta_res;
    X.theta(X.theta>X2.theta_sat) = X2.theta_sat(X.theta>X2.theta_sat);
end
M = 0.018;
sat_index = find(X.h >= X2.he);
k(sat_index, 1) = X2.ksat(sat_index, 1);
...
if ~X.bc
    dhrdT =  -((exp((g.* M .* X.h) ./...
        (R .* (273.15 + X.T(:,i-1)))) .* g .* M .* X.h) ./(R .* (273.15 + X.T(:,i-1)) .^ 2));
end
dhrdh(sat_index, 1) = 0;
dhrdT(sat_index, 1) = 0;
hr(sat_index, 1) = 1;
...
[kH,thao,thaol,Dv] = mex_soil2(X.theta, theta_sat, X.T);
...
X3=struct('h',X.h,'T',X.T,'cvsat',cvsat,'hr',hr,'s',s,'Ep',X.Ep,...
    ...
    'theta_sat',X2.theta_sat,'theta_res',X2.theta_res,'lamda',X2.lamda,'yeeta',X2.yeeta,'ksat',X2.ksat,'itm',X.itm,'qprec',X.qprec,'treeheight',X.treeheight);
...
dalphadT = exp(-(X4.aca ./ (X.T+273.15) ./ (X.T+273.15) + -X4.acb ./ (X.T + 273.15) + X4.acc)) .*...
    (2 .* X4.aca .* (X.T+273.15) ./ ((X.T+273.15) .^ 4) - X4.acb ./ ((X.T + 273.15) .^ 2));
...
Div = Dv0.*10.^5./101325.*((X.T+273.15)./273.15).^1.88.*(X4.alphadiff).^nD.*(X2.theta_sat-X.theta).*thao;
