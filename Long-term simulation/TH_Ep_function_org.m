...
qevap=Ep;%levenberg-marquardt,'StepTolerance',1e-10
% syms x01 x02
...
while true
    ...
    hs(1)=(-qevap-k(1))/k(1)*dz/2+h(1);%hr(1);%(hra+hr(1))/2;%hr(1);
    hrs(1,1)=exp(hs(1)*M*g/(x0+273.15)/R);%log(hrs(1,j))*R*(Ts(1,j)+273.15)/g/M;
...
hrs(1,1)=exp(hs(1)*M*g/(T11+273.15)/R);%log(hrs(1,j))*R*(Ts(1,j)+273.15)/g/M;
TTHH(2)=hrs(1,1);
end
