...
if X3.itm
    ...
    if ~X3.frozen_flag
        ...
        if hs_flag
            ...
            if ~X.bc
                thetas = X2.theta_res(1)+ (X2.theta_sat(1)- X2.theta_res(1))./ (1+(abs(X2.alp(1).*hs)).^X2.N(1)).^X2.M(1).*X.scalingfactor(1);
            end
        ...
        %hs(1)=hcria;
        if  hs<=X3.hcria %To be specific, this upper calculation is based on Ep or E is unknown, otherwise, using 'TH_Ep_function' instead.
            %error(" ")
            Ep_flag=0;
            isofun_hrs3_flag=0;
            hs=X3.hcria;
            if X.theta(1) == X3.theta_sat(1)
