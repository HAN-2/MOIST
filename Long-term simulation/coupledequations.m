...
global out_X out_X1 out_X2 out_X4 out_X3 month
...
X.cil = y4;
if ~X.bc
    X.theta = X2.theta_res+ X.scalingfactor.*(X2.theta_sat- X2.theta_res)./ (1+(abs(X2.alp.*y1)).^X2.N).^X2.M;
    X.theta(X.theta>X2.theta_sat) = X2.theta_sat(X.theta>X2.theta_sat);
else
    error('bc content is not typed')
end
out_X.h = y1;
out_X.T = y2;
out_X.cim = y3;
...
[X1,X3,X4] = parameter_function(X,X2,i);
...
if any(month == [1,2,3,4,])
    ...
    dydt(2*X.n+1:3*X.n,1) = X1.mass_transfer_coefficient ./ X1.coe .* (X.cil(:,i-1) - X.cim(:,i-1));
