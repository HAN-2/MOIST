function [TTHH,of,qevap,Sensibleheat,latentheat,G,rbh,rbw,it] = TH_frozen_function(Tinitial,hrinitial,X3)
...
while true
    [rbh,rbw]=AeroRes(x0,X3.Ta,X3.u,X3.Cp,X3.root_flag,X3.rs(1),X3);
    T11=(X3.Rnet+X3.Cp/rbh*(X3.Ta+273.15)+...
        X3.kH(1)/(X3.dz/2)*(X3.T(1,i-1)+273.15)-...
        X3.Cp/rbh*273.15-X3.rho*0*10^6*qevap*2.501-X3.kH(1)/(X3.dz/2)*273.15)/(X3.Cp/rbh-X3.rho*10^6*qevap*2.361*10^(-3)+X3.kH(1)/(X3.dz/2));

    hs(1)=(-qevap-X3.k(1))/X3.k(1)*X3.dz/2+X3.h(1,i-1);%hr(1);%(hra+hr(1))/2;%hr(1);
    ...
    x01=(X3.Rnet+X3.Cp/rbh*X3.Ta-...
        1e6*X3.rho*0*2.501*((0.001*exp(31.3716-6014.79/(x0+273.15)-0.00792495*(x0+273.15))/(x0+273.15)*(exp(hs(1)*X3.M*X3.g/(x0+273.15)/X3.R))-roa)/(rbh+rbw)/1000)+...
        X3.kH(1)/(X3.dz/2)*X3.T(1,i-1))/...
        (X3.Cp/rbh-X3.rho*0*1e6*((0.001*exp(31.3716-6014.79/(x0+273.15)-0.00792495*(x0+273.15))/(x0+273.15)*(exp(hs(1)*X3.M*X3.g/(x0+273.15)/X3.R))-roa)/(rbh+rbw)/1000)*2.631*1e-3+X3.kH(1)/(X3.dz/2));
