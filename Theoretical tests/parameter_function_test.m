...
thao =0.67;% ((X2.theta_sat-X.theta(:,i-1)).^(7/3))./((X2.theta_sat).^2);
thaol=0.67;%((X.theta).^(7/3))./((X2.theta_sat).^2);%0.66*(theta./thetasat).^(8/3);
...
nDs=0.67+0.33*exp(1-thetas(1)/X2.theta_res(1));
nD =1 ;
nDs = 1;
...
nk = 1;%((thetas-X2.theta_res(1))*0.5+(X2.theta_sat(1)-thetas)*1)/(X2.theta_sat(1)-X2.theta_res(1));
...
X1=struct('k',k,'S', S,'C', C,'theta', X.theta,'cvsat', cvsat,'dcvsatdT', dcvsatdT,'hr', hr,...
    ...
    'qvs',qvs,'G',G,'alphadiff',X4.alphadiff,'qevap',qevap,'cvs',cvs,'Ts',Ts,'hrs',hrs,'cvsats',cvsats);
