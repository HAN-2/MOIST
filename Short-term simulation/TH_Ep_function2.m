function [TTHH,fval_Ep,qevap,Sensibleheat,latentheat,G,rbh,rbw] = TH_Ep_function2(Tinitial,hrinitial,X3)
...
while true
    ...
    hs(1)=(-qevap-X3.k(1))/X3.k(1)*X3.dz/2+X3.h(1);
    hrs(1,1)=exp(hs(1)*X3.M*X3.g/(x0+273.15)/X3.R);

    roa=0.001*exp(31.3716-6014.79/(X3.Ta+273.15)-0.00792495*(X3.Ta+273.15))/(X3.Ta+273.15)*hra;
...
flag=true;
while flag
    thetas=(hs(1)/X3.he(1))^(-X3.lamta(1))*(X3.theta_sat(1)-X3.theta_res(1))+X3.theta_res(1);
    theta1=(X3.h(1)/X3.he(1))^(-X3.lamta(1))*(X3.theta_sat(1)-X3.theta_res(1))+X3.theta_res(1);

    ks(1)=X3.ksat(1)*((thetas(1)-X3.theta_res(1))/(X3.theta_sat(1)-X3.theta_res(1)))^(X3.yeeta(1));
    ke=(ks(1)+X3.k(1))/2; %BA2

    hrs=exp(hs*X3.M*X3.g/(T11+273.15)/X3.R);

    h_s(1)=((-(qevap-X3.Dv(1)/(X3.dz/2)*X3.cvsat(1)*(X3.hr(1)-hrs)-X3.Dv(1)/(X3.dz/2)*X3.s(1)*X3.hr(1)*(X3.T(1,i-1)+0-(T11+0))))-(ke))/(ke)*X3.dz/2+X3.h(1);%hr(1);%(hra+hr(1))/2;%hr(1);
...
hrs(1,1)=exp(hs(1)*X3.M*X3.g/(T11+273.15)/X3.R);
TTHH(2)=hrs(1,1);
end
