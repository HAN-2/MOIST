...
[rbh,rbw]=AeroRes(X3.Ta,X3.Ta,X3.u,X3.Cp,X3.root_flag,X3.rs(1));
if isempty(hrcria)
...
y1=@(x01) X3.Rnet-X3.Cp/X3.rbh*(x01+273.15)+X3.Cp/rbh*(X3.Ta+273.15)-X3.rho*10^6*(-(qvs_hr-qvs_T*(X3.T(1,i-1)-(x01))-(X3.qls)))*2.501+X3.rho*10^6*(-(qvs_hr-qvs_T*(X3.T(1,i-1)-(x01))-(qls)))*(2.361*10^(-3)*x01)+X3.kH(1)/(X3.dz(1)/2)*(X3.T(1,i-1))-X3.kH(1)/(X3.dz(1)/2)*(x01);

sol1=-((0.00021177467174925878*(-1*(9444*X3.dz*qvs_T*rbh(1)*X3.rho*(X3.Cp*X3.dz*X3.Ta-2.501*10^6*X3.dz*qls*rbh(1)*X3.rho+2.501*10^6*X3.dz*qvs_hr*rbh(1)*X3.rho-2.501*10^6*X3.dz*qvs_T*rbh(1)*X3.rho*X3.T(1,i-1)+X3.dz*rbh(1)*X3.Rnet+...
    2*X3.kH(1)*rbh(1)*X3.T(1,i-1))+(-1*X3.Cp*X3.dz+2361*X3.dz*qls*rbh(1)*X3.rho-2361*X3.dz*qvs_hr*rbh(1)*X3.rho+2361*X3.dz*qvs_T*rbh(1)*X3.rho*X3.T(1,i-1)+2.501*10^6*X3.dz*qvs_T*rbh(1)*X3.rho-...
    2*X3.kH(1)*rbh(1))^2)^0.5+X3.Cp*X3.dz-2361*X3.dz*qls*rbh(1)*X3.rho+2361*X3.dz*qvs_hr*rbh(1)*X3.rho-2361*X3.dz*qvs_T*rbh(1)*X3.rho*X3.T(1,i-1)-2.501*10^6*X3.dz*qvs_T*rbh(1)*X3.rho+...
    2*X3.kH(1)*rbh(1)))/(X3.dz*qvs_T*rbh(1)*X3.rho));


qevap=-(qvs_hr-qvs_T*(X3.T(1,i-1)-(sol1))-(qls));
T11=sol1;

lamtaE_s=(2.501-2.361*10^(-3)*T11)*10^6;
Sensibleheat=X3.Cp/rbh*((T11+273.15)-(X3.Ta+273.15));%Cp/rbh*((x0(1)+273.15)-(Ta+273.15))
latentheat=X3.rho*lamtaE_s*qevap;%rho*(2.501-2.361*10^(-3)*x0(1))*10^6/rbw(1)*(x0(2)*(cvsat(1)+s(1)*(x0(1)+0-(T(1)+0)))-cva)
G=-X3.kH(1,1)/(X3.dz/2)*((X3.T(1,i-1)+273.15)-(T11+273.15)); %heat flux into soil-kH(1)/(dz(1)/2)*((T(1)+273.15)-(x0(1)+273.15));
TTHH(1,1)=T11;
TTHH(2,1)=hrcria;
