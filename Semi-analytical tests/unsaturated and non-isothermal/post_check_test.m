...
theta_(h_<X2.he,1)=(h_(h_<X2.he,1)./X2.he).^(-X2.lamda).*(X2.theta_sat-X2.theta_res)+X2.theta_res;
theta_(h_>=X2.he,1) = out_X2.theta_sat;
...
if ~isempty(index)
    ...
    if out_X1.q(n+1)<0
        for i = index(1):index(end)
            h_(i,1) = (out_X1.ql(n+1)-(out_X2.ksat))/(-(out_X2.ksat))*dz+h_(i-1,1);
        end
    end
end
